...
export const HIGHLIGHT_CLASS = "highligh--highlighted"
export const DELETED_CLASS = "highlighter--deleted"

export function highlight(selString, container, selection, bg, color, highlightIndex, version = 0.1) {
  const highlightInfo = {
    version,
    bg: bg ? bg : "yellow",
...
function _recursiveWrapper(
  ...
  container.contents().each((_index, element) => {
    ...
    highlightNode.classList.add(
      bg === "inherit" ? DELETED_CLASS : HIGHLIGHT_CLASS
    )
    highlightNode.style.backgroundColor = bg
